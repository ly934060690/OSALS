...
<template>
  <el-table
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <!--<i class="el-icon-time"></i>-->
        <span style="margin-left: 10px">{{ scope.row.order.name }}</span>
      </template>
    </el-table-column>
        ...
        <el-dialog title="货物" :visible.sync="dialogFormVisible">
          <el-form :model="Form">
            ...
            <el-form-item label="order" :label-width="formLabelWidth">
              <el-input v-model="Form.order.name" autocomplete="off"></el-input>
            </el-form-item>
          </el-form>
...
<script>
  export default {
    ...
    methods: {
      ...
      postForm() {
        const url = this.HOST + '/ordergoods/save';
        this.dialogFormVisible = false;

        var params = new URLSearchParams();
        params.append('name', this.Form.name);
        params.append('number', this.Form.number);
        params.append('order.name', this.Form.order.name);
      ...
      handleDelete(index, row) {
        ...
        console.log(Id);
        this.$axios
          .delete(this.HOST + '/ordergoods/delete/' + Id)
          .then(res => {
            console.log(res);
    ...
    created() {
      this.$axios.get(this.HOST+'/ordergoods/all')
        .then(response=>{
